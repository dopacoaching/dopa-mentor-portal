...
export default function AdminPaymentsPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-dopa-light border border-dopa-green/20 rounded-xl p-4 flex items-center gap-3">
        <div className="w-10 h-10 bg-dopa-green rounded-xl flex items-center justify-center">
          <DollarSign className="w-5 h-5 text-white" />
        </div>
        <div>
          <p className="text-sm text-gray-600">Total Payout — {MONTHS[month - 1]} {year}</p>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {loading ? (
                ...
                <tr><td colSpan={8} className="text-center py-12 text-gray-400">No payment data</td></tr>
              ) : (
                payments.map((p) => (
                  <tr key={p.mentorId} className="hover:bg-gray-50 cursor-pointer" onClick={() => setSelected(p)}>
            ...
            <div className="space-y-3 text-sm">
              {[
                ...
                <div key={row.label} className="flex justify-between py-2 border-b last:border-0">
                  ...
                  <span className={`font-medium ${row.value > 0 ? 'text-green-700' : 'text-gray-400'}`}>₹{row.value.toLocaleString()}</span>
                </div>
              ))}
              <div className="flex justify-between py-3 font-bold text-base border-t-2 mt-2">
                <span>Total</span>
                <span className="text-dopa-green">₹{selected.total.toLocaleString()}</span>
              </div>
              <div className="grid grid-cols-2 gap-2 bg-gray-50 rounded-lg p-3 text-xs text-gray-500">
